...
export async function POST(request: NextRequest) {
  try {
    ...
    // 3) Create bill on Zero Tracking.
    //    App đã gắn cố định 1 tài khoản ngân hàng khi tạo → Zero Tracking tự resolve
    //    (app.payment_account_id), KHÔNG cần gửi payment_account_id per-checkout.
    let billResponse;
    try {
    ...
    // 5) Persist subscription (with coupon; payee resolved by Zero Tracking app-fixed account)
    try {
      const sql = getSql();
      await sql`
