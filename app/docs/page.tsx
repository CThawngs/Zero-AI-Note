  ...
  vi: {
    ...
    tech: {
      ...
      rows: [
        ...
        { label: 'Billing',   value: 'Zero Tracking API VietQR Napas EMVCo (Tự động kích hoạt Realtime & quét mã không cần bấm), Hệ thống Coupon chiết khấu % tự động' },
      ],
    },
    security: {
    ...
    api: {
      ...
      rows: [
        ...
        { method: 'POST', path: '/api/billing/create-invoice', desc: 'Tạo hóa đơn Zero Tracking VietQR Napas EMVCo / Kích hoạt coupon 100% (0đ).' },
        { method: 'GET',  path: '/api/billing/check-status',   desc: 'Tự động thăm dò trạng thái hóa đơn thời gian thực (Zero-Click polling).' },
        { method: 'POST', path: '/api/billing/confirm', desc: 'Xác nhận thanh toán và nâng cấp gói subscription qua Zero Tracking.' },
        { method: 'POST', path: '/api/billing/webhook', desc: 'Webhook nhận thông báo thanh toán thành công (bill.paid) từ Zero Tracking.' },
        { method: 'POST', path: '/api/billing/validate-coupon', desc: 'Kiểm tra tính hợp lệ và tính mức chiết khấu của mã khuyến mãi.' },
  ...
  en: {
    ...
    tech: {
      ...
      rows: [
        ...
        { label: 'AI Engine', value: 'Google GenAI (Gemini 2.5 Flash), BYOK AI Providers (OpenAI, Anthropic, OpenRouter, Groq, NVIDIA, Ollama & Multi-Server Custom Endpoints)' },
        { label: 'Billing',   value: 'Zero Tracking API VietQR Napas EMVCo (Real-time auto-detection & zero-click payment verification), Dynamic percentage coupon system' },
      ],
    },
    security: {
    ...
    api: {
      ...
      rows: [
        ...
        { method: 'POST', path: '/api/billing/create-invoice', desc: 'Create Zero Tracking VietQR Napas EMVCo invoice / 100% coupon instant activation.' },
        { method: 'GET',  path: '/api/billing/check-status',   desc: 'Real-time background invoice status polling (Zero-Click verification).' },
        { method: 'POST', path: '/api/billing/confirm', desc: 'Confirm payment and resolve subscription upgrade with Zero Tracking.' },
        { method: 'POST', path: '/api/billing/webhook', desc: 'Webhook handler for payment completed events (bill.paid) from Zero Tracking.' },
        { method: 'POST', path: '/api/billing/validate-coupon', desc: 'Validate promo coupon code and compute plan discount amounts.' },
