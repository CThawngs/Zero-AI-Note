...
export const ChatScreen: React.FC = () => {
  ...
  // Pagination for chat messages: initial view shows 10 latest messages
  ...
  const isUserLoadingMoreRef = useRef<boolean>(false);

  // Reset pagination when active session changes
  useEffect(() => {
  ...
  // Auto-scroll to bottom only when new messages arrive or processing starts, not when user expands older messages
  useEffect(() => {
    if (isUserLoadingMoreRef.current) {
      isUserLoadingMoreRef.current = false;
      return;
    }

    if (chatMessages.length > prevMessagesLengthRef.current || isProcessingChat) {
      scrollToBottom();
    }
    prevMessagesLengthRef.current = chatMessages.length;
  }, [chatMessages.length, isProcessingChat, processingStep]);

  // Message slicing calculations
  const totalMessages = chatMessages.length;
  const visibleMessages = chatMessages.slice(Math.max(0, totalMessages - visibleMessagesCount));
  const hiddenCount = Math.max(0, totalMessages - visibleMessagesCount);
  const nextBatch = Math.min(10, hiddenCount);

  // Handle loading 10 more older messages with scroll position retention
  const handleLoadMore = () => {
    isUserLoadingMoreRef.current = true;
    const container = scrollContainerRef.current;
    const prevScrollHeight = container ? container.scrollHeight : 0;
    const prevScrollTop = container ? container.scrollTop : 0;

    setVisibleMessagesCount(prev => Math.min(prev + 10, totalMessages));
      ...
      <div className={`flex-1 flex flex-col h-full overflow-hidden min-w-0 ${isArtifactOpen ? 'hidden md:flex' : 'flex'}`}>
        ...
        >
          <div className="max-w-3xl mx-auto space-y-6">
            {/* View More Older Messages Button when there are older messages */}
            {hiddenCount > 0 && (
              <motion.div 
                initial={{ opacity: 0, y: -8 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.25 }}
                className="flex flex-col items-center justify-center py-2 pb-1"
              >
                <motion.button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-2xl text-xs font-bold border transition-all cursor-pointer shadow-xs ${
                    isDark 
                      ? 'bg-[var(--bg-card)] border-[var(--border-color)] hover:border-[var(--accent-primary)]/50 hover:bg-[var(--bg-hover)] text-[var(--text-secondary)] hover:text-[var(--accent-primary)]' 
                      : 'bg-white border-[var(--border-color)] hover:border-[var(--accent-primary)] hover:bg-[var(--accent-subtle)] text-[var(--text-secondary)] hover:text-[var(--accent-primary)] shadow-2xs'
                  }`}
                >
                  <ChevronUp className="w-3.5 h-3.5 text-[var(--accent-primary)]" />
                  <span>
                    {language === 'vi' 
                      ? `Xem thêm ${nextBatch} tin nhắn cũ hơn (${hiddenCount} tin nhắn trước đó)` 
                      : `View ${nextBatch} older messages (${hiddenCount} remaining)`}
                  </span>
                </motion.button>
              </motion.div>
            )}
            ...
            {hiddenCount === 0 && totalMessages > 10 && (
              <motion.div 
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="flex items-center justify-center gap-2 py-3 text-[11px] text-[var(--text-muted)] font-medium select-none"
              >
                <div className="h-px bg-[var(--border-color)] flex-1 max-w-[80px]" />
                <span className="flex items-center gap-1 opacity-75">
                  <Clock className="w-3 h-3 text-[var(--accent-primary)]" />
                  {language === 'vi' ? 'Bắt đầu cuộc trò chuyện' : 'Beginning of conversation'}
                </span>
                <div className="h-px bg-[var(--border-color)] flex-1 max-w-[80px]" />
              </motion.div>
            )}
            ...
            {/* Chat Message List (Visible slice of messages) */}
