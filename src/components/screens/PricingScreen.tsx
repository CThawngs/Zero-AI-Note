        ...
        <div className="pt-10 space-y-6">
          <div className="text-center space-y-2">
            <h2 className="text-xl sm:text-2xl font-bold text-[var(--text-primary)]">
              {language === 'vi' ? 'Bảng So Sánh Quyền Lợi Chi Tiết' : 'Detailed Feature Comparison'}
            </h2>
            <p className="text-xs text-[var(--text-secondary)]">
              {language === 'vi' ? 'Đối chiếu đầy đủ hạn mức & tính năng chuyên sâu giữa 3 gói' : 'Compare all quotas and deep academic features across tiers'}
            </p>
          </div>

          <div className="rounded-3xl border overflow-hidden bg-[var(--bg-card)] border-[var(--border-color)] shadow-sm overflow-x-auto">
            <table className="w-full text-left text-xs border-collapse min-w-[600px]">
              <thead>
                <tr className="border-b border-[var(--border-color)] bg-[var(--bg-hover)]">
                  <th className="p-4 font-bold text-[var(--text-primary)] w-2/5">
                    {language === 'vi' ? 'Tính Năng & Quyền Lợi' : 'Features & Benefits'}
                  </th>
                  <th className="p-4 font-bold text-center text-[var(--text-secondary)] w-1/5">Free</th>
                  <th className="p-4 font-bold text-center text-blue-500 w-1/5">Pro</th>
                  <th className="p-4 font-bold text-center text-[var(--accent-primary)] w-1/5">Ultra</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[var(--border-color)]">
                <tr>
                  <td className="p-4 font-medium text-[var(--text-primary)]">{language === 'vi' ? 'Hạn mức lưu trữ Note' : 'Note Storage Limit'}</td>
                  <td className="p-4 text-center font-semibold text-[var(--text-secondary)]">20 Notes</td>
                  <td className="p-4 text-center font-bold text-blue-500">50 Notes</td>
                  <td className="p-4 text-center font-black text-[var(--accent-primary)]">{language === 'vi' ? 'Không giới hạn (∞)' : 'Unlimited (∞)'}</td>
                </tr>
                <tr>
                  <td className="p-4 font-medium text-[var(--text-primary)]">{language === 'vi' ? 'Số lượng Templates Học Thuật' : 'Academic Templates'}</td>
                  <td className="p-4 text-center font-semibold text-[var(--text-secondary)]">3 templates</td>
                  <td className="p-4 text-center font-bold text-blue-500">9 templates</td>
                  <td className="p-4 text-center font-black text-[var(--accent-primary)]">17 templates</td>
                </tr>
                <tr>
                  <td className="p-4 font-medium text-[var(--text-primary)]">{language === 'vi' ? 'Hạn mức Custom Templates' : 'Custom Templates Quota'}</td>
                  <td className="p-4 text-center font-semibold text-[var(--text-secondary)]">5 templates</td>
                  <td className="p-4 text-center font-bold text-blue-500">25 templates</td>
                  <td className="p-4 text-center font-black text-[var(--accent-primary)]">{language === 'vi' ? 'Không giới hạn (∞)' : 'Unlimited (∞)'}</td>
                </tr>
                <tr>
                  <td className="p-4 font-medium text-[var(--text-primary)]">{language === 'vi' ? 'Chế độ Xem Trước (Preview)' : 'Preview Mode'}</td>
                  <td className="p-4 text-center text-[var(--text-secondary)]">Raw / Markdown</td>
                  <td className="p-4 text-center text-blue-500 font-medium">Static HTML (CSS)</td>
                  <td className="p-4 text-center text-[var(--accent-primary)] font-bold">Interactive HTML (JS)</td>
                </tr>
                <tr>
                  <td className="p-4 font-medium text-[var(--text-primary)]">{language === 'vi' ? 'Xuất File & Định Dạng' : 'Export Formats'}</td>
                  <td className="p-4 text-center text-[var(--text-secondary)]">PDF, DOCX, MD</td>
                  <td className="p-4 text-center text-blue-500 font-medium">PDF, DOCX, MD, HTML</td>
                  <td className="p-4 text-center text-[var(--accent-primary)] font-bold">Tất cả + ZIP Bundle</td>
                </tr>
                <tr>
                  <td className="p-4 font-medium text-[var(--text-primary)]">{language === 'vi' ? 'Tự kết nối AI riêng (BYOK)' : 'Connect Own AI (BYOK)'}</td>
                  <td className="p-4 text-center text-emerald-500 font-bold"><Check className="w-4 h-4 mx-auto" /></td>
                  <td className="p-4 text-center text-emerald-500 font-bold"><Check className="w-4 h-4 mx-auto" /></td>
                  <td className="p-4 text-center text-emerald-500 font-bold"><Check className="w-4 h-4 mx-auto" /></td>
                </tr>
                <tr>
                  <td className="p-4 font-medium text-[var(--text-primary)]">{language === 'vi' ? 'Tốc độ Xử Lý & Hỗ Trợ' : 'Speed & Support'}</td>
                  <td className="p-4 text-center text-[var(--text-muted)]">{language === 'vi' ? 'Tiêu chuẩn' : 'Standard'}</td>
                  <td className="p-4 text-center text-blue-500 font-medium">{language === 'vi' ? 'Ưu tiên' : 'Priority'}</td>
                  <td className="p-4 text-center text-[var(--accent-primary)] font-bold">{language === 'vi' ? 'Cao cấp nhất 24/7' : 'Top VIP 24/7'}</td>
                </tr>
              </tbody>
            </table>
          </div>
