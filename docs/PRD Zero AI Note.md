...
- **`web_search(query)` — Tavily**: tin tức/sự kiện/giá cả thời gian thực. Free 1.000 credit/tháng (đã verify tavily.com/pricing), key server-side `TAVILY_API_KEY` (ĐÃ cấu hình trên Vercel 23/08). Guard quota: đếm `usage` operation=`tavily_search` trong tháng, chặn ở 950 (buffer 50) → agent trả lời "không thể tra cứu web lúc này" thay vì lỗi cứng.
...
- **Fallback chain khi Gemini rate-limit** (23/08): Gemini agent-loop → OpenRouter agent-loop VẪN GIỮ tools (`openrouter/free`, format FC OpenAI-compatible) → OpenRouter chatOnly thuần → legacy engines. Identity luôn khai báo trung thực engine đang chạy.
